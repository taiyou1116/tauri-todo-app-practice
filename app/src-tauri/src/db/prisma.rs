    ...
    impl Data {
        pub fn todos(&self) -> Result<&Vec<super::todo_item::Data>, String> {
            match self.todos.as_ref() {
                Some(v) => Ok(v),
                None => Err(
                    "Attempted to access todos but did not fetch it using the .with() syntax"
                        .to_string(),
                ),
            }
        }
                    ...
                    inputs: if where_params.len() > 0 {
                        vec![Input {
                            ...
                            fields: where_params
                                .into_iter()
                                .map(|param| {
                                    Into::<super::todo_item::WhereParam>::into(param).to_field()
                                })
                                .collect(),
                            ..Default::default()
                        }]
...
pub mod todo_item {
    ...
    impl Data {
        pub fn todo_list(&self) -> Result<&super::todo_list::Data, String> {
            match self.todo_list.as_ref() {
                Some(v) => Ok(v),
                None => Err(
                    "Attempted to access todo_list but did not fetch it using the .with() syntax"
                        .to_string(),
                ),
            }
        }
    ...
    pub enum SetParam {
        ...
        SetText(String),
        SetComplete(bool),
        SetDeadline(Option<chrono::DateTime<chrono::Utc>>),
    }
    impl SetParam {
        pub fn to_field(self) -> Field {
            match self {
    ...
    pub enum WhereParam {
        ...
        TextNot(String),
        CompleteEquals(bool),
        DeadlineEquals(Option<chrono::DateTime<chrono::Utc>>),
        DeadlineInVec(Vec<chrono::DateTime<chrono::Utc>>),
        DeadlineNotInVec(Vec<chrono::DateTime<chrono::Utc>>),
        DeadlineLt(chrono::DateTime<chrono::Utc>),
        DeadlineLte(chrono::DateTime<chrono::Utc>),
        DeadlineGt(chrono::DateTime<chrono::Utc>),
        DeadlineGte(chrono::DateTime<chrono::Utc>),
        DeadlineNot(chrono::DateTime<chrono::Utc>),
    }
    impl WhereParam {
        pub fn to_field(self) -> Field {
            match self {
                ...
                Self::DeadlineEquals(value) => Field {
                    name: "deadline".into(),
                    fields: Some(vec![Field {
                        name: "equals".into(),
                        value: Some(serde_json::to_value(value).unwrap()),
                ...
                Self::DeadlineInVec(value) => Field {
                    name: "deadline".into(),
                    fields: Some(vec![Field {
                        name: "in".into(),
                        list: true,
                ...
                Self::DeadlineNotInVec(value) => Field {
                    name: "deadline".into(),
                    fields: Some(vec![Field {
                        name: "notIn".into(),
                        list: true,
